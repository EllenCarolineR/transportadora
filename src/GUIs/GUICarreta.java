...
public class GUICarreta extends JDialog {
    ...
    private CarretaTableModel tableModel;
    DAOCarreta daoCarreta = new DAOCarreta();

    public GUICarreta() {

        setTitle("CRUD - Carreta");
        ...
        List<Carreta> lista = new ArrayList<>();
        tableModel = new CarretaTableModel(lista);
...
//        painelBotoes.add(btnRem);
            ...
            public void actionPerformed(ActionEvent evt) {
                if (table.getSelectedRow() >= 0) {
                    ...
                    if (JOptionPane.YES_OPTION == JOptionPane.showConfirmDialog(cp,
                            "Confirme a exclusão [" + tableModel.getValue(table.getSelectedRow()).getIdCarreta() + " - "
                            + tableModel.getValue(table.getSelectedRow()).getCapacidadeMax()+ "]?", "Confirm",
        ...
        btnAdd.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                carreta.setIdCarreta(daoCarreta.autoIdCarretaString());
                carreta.setCapacidadeMax(0);

                try {
                    Carreta c = daoCarreta.obter(carreta.getIdCarreta());
                    if (c == null) {
                        daoCarreta.inserir(carreta);
                        tableModel.onAdd(carreta);
                        tableModel.fireTableDataChanged();
                    } else {
                        JOptionPane.showMessageDialog(cp, "Já existe");
                    }
                } catch (Exception err) {
                    JOptionPane.showMessageDialog(cp, "Erro ao inserir =>" + carreta.getIdCarreta()
        ...
        btnRem.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (table.getSelectedRow() != -1 && table.getSelectedRow() < tableModel.getRowCount()) {
                    Carreta c = tableModel.getValue(table.getSelectedRow());
                    daoCarreta.remover(c);
                    tableModel.onRemove(table.getSelectedRows());
                } else {
                    JOptionPane.showMessageDialog(cp, "Escolha na tabela a carreta a ser excluída");
                    table.requestFocus();
                }
                tableModel.fireTableDataChanged();
            }
        ...
        btnCarregar.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    List<Carreta> lc = daoCarreta.list();
